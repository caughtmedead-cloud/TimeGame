...
public enum SECTR_MemberUpdateMode { Realtime, Delayed, OnSaveOnly }
...
public class SECTR_Member : MonoBehaviour
{
        ...
        public void Init(GameObject gameObject, Renderer renderer, Light light, Terrain terrain, SECTR_Member member, bool dirShadowCaster, Vector3 shadowVec)
        {
            ...
            if (SECTR_Modules.VIS)
            {
                ...
                else
                {
                    ...
                    shadowLightRange = 0;
                    shadowLightType = LightType.Rectangle;
                    shadowCullingMask = 0;
                }
            }
            else
    ...
    private double nextLateUpdateTimeStamp = 0;
...
#if UNITY_EDITOR
        //load these static values from Editor prefs
        MemberUpdateMode = (SECTR_MemberUpdateMode)EditorPrefs.GetInt("PW.SECTR_MemberUpdateMode");
        MemberUpdateDelay = EditorPrefs.GetInt("PW.SECTR_MemberUpdateDelay");
    ...
    {
        if (SECTR_Member.MemberUpdateMode == SECTR_MemberUpdateMode.OnSaveOnly && gameObject.scene == scene)
        {
            UpdateMembers();
        }
    }
    ...
    {
#if UNITY_EDITOR
        //Always update when playing, or realtime, or delayed and over the delay threshold
        if (Application.isPlaying ||
            SECTR_Member.MemberUpdateMode == SECTR_MemberUpdateMode.Realtime ||
            (SECTR_Member.MemberUpdateMode == SECTR_MemberUpdateMode.Delayed && nextLateUpdateTimeStamp <= SECTR_SectorUtils.GetUnixTimeStamp()))
        {
            //Add a slight randomness so that not all updates fire at the same time
            nextLateUpdateTimeStamp = SECTR_SectorUtils.GetUnixTimeStamp() + (long)UnityEngine.Random.Range(SECTR_Member.MemberUpdateDelay * 0.75f, SECTR_Member.MemberUpdateDelay * 1.25f);
#endif
            UpdateMembers();
    ...
    /// <summary>
    /// Updates the preference settings on the component
    /// </summary>
