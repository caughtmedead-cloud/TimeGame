...
public class ZonePrefabSpawner : EditorWindow
{
    ...
    [MenuItem("Tools/Zones/Zone Spawner")]
    public static void ShowWindow()
    {
        ZonePrefabSpawner window = GetWindow<ZonePrefabSpawner>("Zone Spawner");
    ...
    private void RefreshPrefabs()
    {
        ...
        foreach (string guid in prefabGuids)
        {
            ...
            if (prefab != null && prefab.GetComponent<BaseZone>() != null)
            {
    ...
    private void OnGUI()
    {
        ...
        EditorGUILayout.LabelField("Zone Prefab Spawner", EditorStyles.boldLabel);
    ...
    {
        bool globalTextEnabled = ZoneVisualizer.GetGlobalTextVisibility();
        bool globalPlayModeEnabled = ZoneVisualizer.GetGlobalPlayModeVisibility();
        ...
        {
            ZoneVisualizer.SetGlobalTextVisibility(globalTextEnabled);
        }
        ...
        if (EditorGUI.EndChangeCheck())
        {
            ZoneVisualizer.SetGlobalPlayModeVisibility(globalPlayModeEnabled);
        }
        EditorGUILayout.EndHorizontal();
    ...
    private void DrawPrefabList()
    {
        ...
        {
            EditorGUILayout.HelpBox("No zone prefabs found. Create zones using the Zone Creator window.", MessageType.Info);
            return;
        }
        ...
        foreach (GameObject prefab in zonePrefabs)
        {
            ...
            BaseZone zone = prefab.GetComponent<BaseZone>();
            if (zone != null)
            {
    ...
    private void UpdatePreview(Vector3 position, Quaternion rotation)
    {
        if (previewInstance == null)
        {
            ...
            {
                if (component is BaseZone || component is ZoneVisualizer)
                    continue;
    ...
    private void DrawPreviewGizmos(Vector3 position, Quaternion rotation, Vector3 surfaceNormal)
    {
        ...
        BaseZone zone = selectedPrefab.GetComponent<BaseZone>();
        if (zone == null)
            return;
    ...
    private void PlacePrefab(Vector3 position, Quaternion rotation)
    {
        ...
        BaseZone zone = instance.GetComponent<BaseZone>();
        if (zone != null)
        {
            instance.name = zone.zoneName;
        }

        Undo.RegisterCreatedObjectUndo(instance, "Place Zone");
