...
namespace Inventory.Scripts.Editor
{
    public class UGIGameObjectCreationEditor : EditorWindow
    {
        ...
        private const string HolderImageGuid = "67e69267f9f8c4f2196bd5d85c3ccbab";
        private const string ContainerNameBackground = "e18ea334520ec8e4e8c19c20dba6cc7b";
        ...
        [MenuItem(UltimateGridInventoryPrefixMenu + "Replicator Holder for Character DF", false, (int)PriorityOrder.Ui)]
        private static void AddNonStorableHolder(MenuCommand menuCommand)
        {
            GameObject go;
            using (new UGIEditorHelper.FactorySwapToEditor())
                go = CreateHolder("ReplicatorHolder", typeof(ReplicatorItemHolder));
        ...
        private static GameObject CreateHolder(string rootName, Type holderType)
        {
            var root = UGIEditorHelper.CreateUIElementRoot(rootName, HolderElementSize,
                typeof(Image),
                holderType,
                typeof(ItemHolderInteract2D),
                typeof(RectMask2D)
            );

            var rootImage = root.GetComponent<Image>();
            rootImage.sprite = GetAssetByGuid<Sprite>(HolderImageGuid);
        ...
        [MenuItem(UltimateGridInventoryPrefixMenu + "Display Filler", false, (int)PriorityOrder.PrefabsLower)]
        ...
        private static GameObject CreateDisplayFiller()
        {
            ...
            var gridFiller = UGIEditorHelper.CreateUIObject("GridFiller", root,
                typeof(GridFiller),
                typeof(RectTransform),
                typeof(HorizontalLayoutGroup)
            );

            var gridParentHlg = gridFiller.GetComponent<HorizontalLayoutGroup>();
            ...
            var gridParentRT = gridFiller.GetComponent<RectTransform>();
        ...
        [MenuItem(UltimateGridInventoryPrefixMenu + "Character Display Filler", false, (int)PriorityOrder.PrefabsLower)]
        private static void AddCharacterDisplayFiller(MenuCommand menuCommand)
        {
            GameObject go;
            using (new UGIEditorHelper.FactorySwapToEditor())
                go = CreateCharacterDisplayFiller();

            UGIEditorHelper.PlaceUIElementRoot(go, menuCommand);
        }

        private static GameObject CreateCharacterDisplayFiller()
        {
            var root = UGIEditorHelper.CreateUIElementRoot("CharacterDisplayFiller", ScrollAreaElementSize,
                typeof(Image),
                typeof(CharacterDisplayFiller),
                typeof(VerticalLayoutGroup),
                typeof(ContentSizeFitter)
            );
            ...
            var refreshLayoutFiller = header.GetComponent<RefreshLayoutFiller>();
            refreshLayoutFiller.UpdateValues(refreshOnRefreshValue: true);

            var horizontalLayoutGroup = header.GetComponent<HorizontalLayoutGroup>();
            horizontalLayoutGroup.childAlignment = TextAnchor.UpperLeft;
            horizontalLayoutGroup.childForceExpandWidth = true;
            horizontalLayoutGroup.childForceExpandHeight = true;

            var contentSizeFitter = header.GetComponent<ContentSizeFitter>();
            contentSizeFitter.horizontalFit = ContentSizeFitter.FitMode.PreferredSize;

            var characterFillerName = UGIEditorHelper.CreateUIObject("CharacterFillerName", header,
                typeof(RectTransform),
                typeof(Image),
                typeof(HorizontalLayoutGroup),
                typeof(ContentSizeFitter)
            ...
            var characterFillerNameRectTransform = characterFillerName.GetComponent<RectTransform>();
            var sizeDelta = characterFillerNameRectTransform.sizeDelta;
            characterFillerNameRectTransform.sizeDelta = new Vector2(sizeDelta.x, 45f);

            var characterNameBackgroundImage = characterFillerName.GetComponent<Image>();
            characterNameBackgroundImage.type = Image.Type.Sliced;
            characterNameBackgroundImage.sprite = GetAssetByGuid<Sprite>(ContainerNameBackground);

            var characterHorizontalLayoutGroup = characterFillerName.GetComponent<HorizontalLayoutGroup>();
            characterHorizontalLayoutGroup.childAlignment = TextAnchor.MiddleCenter;
            characterHorizontalLayoutGroup.padding = new RectOffset(46, 46, 1, 1);
            characterHorizontalLayoutGroup.childControlWidth = true;
            characterHorizontalLayoutGroup.childControlHeight = true;

            var characterNameFillerFitter = characterFillerName.GetComponent<ContentSizeFitter>();
            characterNameFillerFitter.horizontalFit = ContentSizeFitter.FitMode.PreferredSize;

            var characterNameText = UGIEditorHelper.CreateUIObject("Text", characterFillerName,
                typeof(RectTransform),
                typeof(TextMeshProUGUI)
            );

            var tmpText = characterNameText.GetComponent<TMP_Text>();
            tmpText.SetText("Character Name");
            ...
            tmpText.margin = new Vector4(0f, 2f, 0f);

            var headerRT = header.GetComponent<RectTransform>();
            RectHelper.SetAnchorsForGrid(headerRT);
            headerRT.pivot = new Vector2(0, 1);

            var characterInventory = UGIEditorHelper.CreateUIObject("Character_Inventory", root,
                typeof(RectTransform),
                typeof(VerticalLayoutGroup),
                typeof(ContentSizeFitter)
            );

            var characterInventoryVlg = characterInventory.GetComponent<VerticalLayoutGroup>();
            characterInventoryVlg.childAlignment = TextAnchor.UpperLeft;
            characterInventoryVlg.padding = new RectOffset(12, 12, 16, 16);
            characterInventoryVlg.spacing = 12;
            characterInventoryVlg.childControlWidth = true;
            characterInventoryVlg.childScaleHeight = true;
            characterInventoryVlg.childForceExpandWidth = true;
            characterInventoryVlg.childForceExpandHeight = true;

            var characterInventoryRT = characterInventory.GetComponent<RectTransform>();
            RectHelper.SetAnchorsForGrid(characterInventoryRT);
            characterInventoryRT.pivot = new Vector2(0, 1);

            var characterInventoryCsf = characterInventory.GetComponent<ContentSizeFitter>();
            characterInventoryCsf.horizontalFit = ContentSizeFitter.FitMode.PreferredSize;
            characterInventoryCsf.verticalFit = ContentSizeFitter.FitMode.PreferredSize;

            var characterReplicatorWeaponHolder = CreateCharacterReplicatorHolder("Weapon_Area", characterInventory);

            var characterReplicatorWeaponHolderRT = characterReplicatorWeaponHolder.GetComponent<RectTransform>();
            var characterHolderAreaSizeDelta = characterReplicatorWeaponHolderRT.sizeDelta;
            characterReplicatorWeaponHolderRT.sizeDelta = new Vector2(characterHolderAreaSizeDelta.x, 164);

            var replicatorItemHolder = characterReplicatorWeaponHolder.GetComponentInChildren<ReplicatorItemHolder>();
            var replicatorItemHolderRT = replicatorItemHolder.GetComponent<RectTransform>();
            replicatorItemHolderRT.sizeDelta = new Vector2(520, 200);

            CreateCharacterReplicatorHolder("Backpack_Area", characterInventory, true);

            return root;
        }

        private static GameObject CreateCharacterReplicatorHolder(string name,
            GameObject characterInventory, bool shouldCreateCharacterGridFiller = false)
        {
            var holderArea = UGIEditorHelper.CreateUIObject(name, characterInventory,
                typeof(RectTransform),
                typeof(VerticalLayoutGroup),
                typeof(ContentSizeFitter),
                typeof(RefreshLayoutFiller)
            ...
            var holderAreaVlg = holderArea.GetComponent<VerticalLayoutGroup>();
            holderAreaVlg.spacing = 4;
            holderAreaVlg.childAlignment = TextAnchor.UpperLeft;
            holderAreaVlg.childForceExpandWidth = false;
            holderAreaVlg.childForceExpandHeight = true;

            var backpackAreaCsf = holderArea.GetComponent<ContentSizeFitter>();
            backpackAreaCsf.verticalFit = ContentSizeFitter.FitMode.PreferredSize;

            if (!shouldCreateCharacterGridFiller)
            {
                holderArea.AddComponent<LayoutElement>();
                var backpackAreaLe = holderArea.GetComponent<LayoutElement>();
                backpackAreaLe.minWidth = 520;
            }

            var backpackAreaRlf = holderArea.GetComponent<RefreshLayoutFiller>();
            backpackAreaRlf.UpdateValues(refreshOnRefreshValue: true);

            var holderName = name.Replace("_Area", "");
            var replicatorContainerHolder =
                CreateHolder(holderName + "_ReplicatorHolder", typeof(ReplicatorItemHolder));
            replicatorContainerHolder.transform.SetParent(holderArea.transform);

            var replicatorItemHolder = replicatorContainerHolder.GetComponent<ReplicatorItemHolder>();

            if (shouldCreateCharacterGridFiller)
            {
                CreateCharacterGridFiller(holderArea, replicatorItemHolder, holderName);
            }

            return holderArea;
        }

        private static GameObject CreateCharacterGridFiller(GameObject replicatorHolderName,
            ReplicatorItemHolder replicatorItemHolder, string holderName = "ReplicatorHolder")
        {
            var gridContainer = UGIEditorHelper.CreateUIObject(holderName + "_GridContainer", replicatorHolderName,
                typeof(RectTransform),
                typeof(Image),
                typeof(CharacterGridFiller),
                typeof(HorizontalLayoutGroup),
                typeof(ContentSizeFitter),
                typeof(LayoutElement)
            );

            var gridContainerImage = gridContainer.GetComponent<Image>();
            gridContainerImage.type = Image.Type.Sliced;
            gridContainerImage.sprite = GetAssetByGuid<Sprite>(ContainerWindowGuid);

            var characterGridFiller = gridContainer.GetComponent<CharacterGridFiller>();
            characterGridFiller.SetHolder(replicatorItemHolder);

            var gridContainerHlg = gridContainer.GetComponent<HorizontalLayoutGroup>();
            gridContainerHlg.childAlignment = TextAnchor.UpperLeft;
            gridContainerHlg.childForceExpandWidth = true;
            gridContainerHlg.childForceExpandHeight = true;

            var gridContainerCsf = gridContainer.GetComponent<ContentSizeFitter>();
            gridContainerCsf.horizontalFit = ContentSizeFitter.FitMode.PreferredSize;
            gridContainerCsf.verticalFit = ContentSizeFitter.FitMode.PreferredSize;

            var gridContainerLe = gridContainer.GetComponent<LayoutElement>();
            gridContainerLe.preferredWidth = 0;
            gridContainerLe.preferredHeight = 0;

            return gridContainer;
        }

        [MenuItem(UltimateGridInventoryPrefixMenu + "Options Controller", false, (int)PriorityOrder.Configuration)]
        ...
        [MenuItem(UltimateGridInventoryPrefixMenu + "Option Button", false, (int)PriorityOrder.Configuration)]
        ...
        {
            contentRT.gameObject.AddComponent<GridFiller>();
        }
        ...
        [MenuItem(UltimateGridInventoryPrefixMenu + "Resident Evil Display Filler", false,
            (int)PriorityOrder.PrefabsLower)]
