...
namespace Inventory.Tests.EditMode
{
    public class GridTableTests
    {
        ...
        public void Place2X2ItemInTheGrid_ShouldInsert()
        {
            ...
            inventoryItem.RefreshItem(CreateItem());

            var firstInsert = _gridTable.PlaceItem(inventoryItem.ItemTable, 0, 0);

            Assert.AreEqual(GridResponse.Inserted, firstInsert);
            Assert.AreEqual(0, inventoryItem.ItemTable.Position.x);
            Assert.AreEqual(0, inventoryItem.ItemTable.Position.y);
        }
        ...
        public void ShouldPickUpTheItem_PickItemIfIsTheSamePos()
        {
            ...
            Assert.AreEqual(1, pickedInventoryItem.Position.x);
            Assert.AreEqual(1, pickedInventoryItem.Position.y);
        }
        ...
        {
            var guidsForInventoryItem =
                AssetDatabase.FindAssets("ItemIcon", new[] { "Assets/Inventory/Scripts/Core/Prefabs" });
        ...
        private static ItemTable CreateItem()
        {
            ...
            return StaticInventoryContext.CreateItemTable(itemDataSo);
        }
    }
}